...
def generate_web_page(ticker: str, desc: str, mode: str, df: pd.DataFrame, score: float, signals: list):
    ...
    # Formattazione data/ora per l'asse X discreto (rimuove i buchi di notte/weekend)
    ...
    # Creazione figura con 3 subplots (Candele, Volumi, RSI)
    fig = make_subplots(
        rows=3, cols=1,
        shared_xaxes=True,
    ...
    # 1. Candele Heikin Ashi / Standard
    fig.add_trace(
        go.Candlestick(
            x=x_dates,
            open=df['Open'],
    ...
    # Medie Mobili (se presenti nel DF)
    ...
    # 2. Volumi (Barre larghe e colorate)
    fig.add_trace(
        go.Bar(
            x=x_dates,
            y=df['Volume'],
    ...
    # 3. RSI (se calcolato o disponibile)
    import ta
    rsi = ta.momentum.rsi(df['Close'], window=9 if mode == 'flash' else 14)
    fig.add_trace(
        go.Scatter(
            x=x_dates,
            y=rsi,
    ...
    # Linee di riferimento RSI (30 e 70)
    ...
    # Layout Dark Mode e Categoriario
    fig.update_layout(
    ...
    # Forzatura Asse X Categoriale (unisce le candele eliminando gli spazi vuoti)
