...
Mostra la previsione sequenziale per:
- Intraday (1H, 2H, 3H, 4H, 5H)
- Daily (1D, 2D, 3D, 4D, 5D)
Invio messaggi Telegram distinti per Portafoglio ETF ed ETF Osservati.
"""
...
class TimesFMPredictor:
    ...
    def predict_sequence(self, series: pd.Series, horizon: int = 5) -> Tuple[List[float], float]:
        ...
        try:
            ...
            predictions = forecast[0][:horizon]
            
            changes_pct = [((float(p) - last_price) / last_price) * 100.0 for p in predictions]
...
def analyze_etf_timesfm(ticker: str, predictor: TimesFMPredictor) -> Tuple[List[float], List[float], float, float, Optional[pd.DataFrame]]:
    ...
    try:
        ...
        if not df_d.empty and len(df_d) >= DAILY_MIN_POINTS:
            ...
            daily_changes, _ = predictor.predict_sequence(df_d['Close'], horizon=5)
        ...
        if not df_h.empty and len(df_h) >= 32:
            ...
            hourly_changes, _ = predictor.predict_sequence(df_h['Close'], horizon=5)
...
def main():
    ...
    # Processa ETF Osservati
    watchlist_results = []
    if watchlist_etf:
        print("\n👁️ ANALISI WATCHLIST ETF")
        for ticker in watchlist_etf:
            h_ch, d_ch, var_pct, price, df_d = analyze_etf_timesfm(ticker, predictor)
            desc = descriptions.get(ticker, ticker)
    ...
    if token and chat_id:
        if portfolio_results:
            ...
            print("\n📩 Invio Telegram Portafoglio ETF...")
