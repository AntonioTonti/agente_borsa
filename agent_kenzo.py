#!/usr/bin/env python3
"""
Agente Kenzo - Analisi Giornaliera con VIX Regime

Differenze rispetto al FLASH:
- Solo timeframe Daily (nessun 1H)
- 12 indicatori (aggiunto vix_regime al 10%)
- Pesi ripartiti: ZigZag 8%, RSI 3%, MACD 4%
- DB separato: kenzo_history.db
...
from vix_utils import get_vix_regime

# ============================================================================
# COSTANTI
# ============================================================================
DB_PATH = "kenzo_history.db"
RISK_PER_TRADE_PCT = 2.0
ORIZZONTE_VERIFICA_GIORNI = 5

# Pesi di default KENZO (con VIX)
DEFAULT_WEIGHTS_KENZO = {
    "ema_ma": 0.15,
    "trend": 0.13,
    "analyst": 0.05,
    "delta_ema_ma": 0.12,
    "ha_force": 0.15,
    "ha_state": 0.10,
    "zigzag": 0.08,         # era 0.10
    "vol": 0.05,
    "close_change": 0.05,
    "rsi": 0.03,            # era 0.05
    "macd": 0.04,           # era 0.05
    "vix_regime": 0.10,     # NUOVO
}
...
def analyze_df_engine(
    ...
    if pesi is None:
        pesi = dict(DEFAULT_WEIGHTS_KENZO)

    # Se vix_data non passato, default neutro
    if vix_data is None:
        vix_data = {'score': 0.5, 'vix_value': None, 'message': "VIX: N/D"}
    ...
    # 11. VIX REGIME
    signals.append(f"😱 {vix_data.get('message', 'VIX: N/D')}")
...
def analyze_kenzo_ticker(
    ...
    try:
        ...
        signals_d, score_d, extra_d, sub_scores_d = analyze_df_engine(
            df_d, tk=tk, pesi=pesi, vix_data=vix_data
        )
        extra_data.update(extra_d)
        extra_data['sub_scores'] = sub_scores_d

        return signals_d, score_d, extra_data, df_d
...
def _build_single_ticker_block(
    ticker: str, score_d: float, extra_data: Dict, descriptions: Dict
) -> str:
    desc = descriptions.get(ticker, ticker)
    bullet = get_bullet(score_d)
    var_pct = extra_data.get('daily_var_pct', 0.0)
    sign = "+" if var_pct > 0 else ""
    url = f"https://antoniotonti.github.io/agente_borsa/kenzo/{ticker}.html"
    risk_d = extra_data.get('risk_daily', {})
    vix_value = extra_data.get('vix_value')
    vix_tag = f" | VIX {vix_value:.1f}" if vix_value is not None else ""

    parts = []
    parts.append(f"🔹 [{ticker}]({url}) - *{desc}* (Oggi: {sign}{var_pct:.2f}%{vix_tag})")
    parts.append("")
    parts.append(f"   📈 *Daily:* {bullet} `{score_d:.3f}`")
...
def create_report_blocks(
    ...
    blocks = []
    for ticker, _, score_d, extra_data, _ in sorted_results:
        blocks.append(_build_single_ticker_block(ticker, score_d, extra_data, descriptions))
    return title, blocks


def create_portfolio_report(results, descriptions):
    now_str = datetime.now().strftime("%H:%M")
    return create_report_blocks(f"💰 *PORTAFOGLIO KENZO ({now_str})*", results, descriptions)


def create_watchlist_report(results, descriptions):
    now_str = datetime.now().strftime("%H:%M")
    return create_report_blocks(f"👁️ *OSSERVATI KENZO ({now_str})*", results, descriptions)
...
# GRUPPO TICKER
# ============================================================================
def process_ticker_group(
    tickers: List[str], categoria: str, descriptions: Dict, vix_data: Dict
) -> List:
    results = []
    if not tickers:
        return results
    pesi = get_pesi_correnti(categoria, DEFAULT_WEIGHTS_KENZO)
    ...
    for ticker in tickers:
        signals_d, score_d, extra_data, df_d = analyze_kenzo_ticker(
            ticker, pesi=pesi, vix_data=vix_data
        )
        # Inietta vix_value per il report
        extra_data['vix_value'] = vix_data.get('vix_value')
        results.append((ticker, signals_d, score_d, extra_data, df_d))

        if df_d is not None and not df_d.empty:
            desc = descriptions.get(ticker, ticker)
            generate_web_page(ticker, desc, "kenzo", df_d, score_d, signals_d)
            ...
            try:
                save_previsione(
                    ...
                    prezzo_emissione=prezzo_emissione, score=score_d,
...
def main():
    ...
    try:
        print("=" * 60)
        print("📊 AGENTE KENZO - ANALISI GIORNALIERA con VIX")
        print(f"Avvio: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}")
        print("=" * 60)

        # Imposta DB path per Kenzo
        set_db_path(DB_PATH)
        init_db()

        # VIX (una volta per run)
        print("\n😱 DOWNLOAD VIX")
        vix_data = get_vix_regime()
        print(f"   VIX: {vix_data['vix_value']} | Score: {vix_data['score']}")
        print(f"   {vix_data['message']}")
        ...
        # Auto-tuning
        print("\n🎯 AUTO-TUNING PESI")
        for cat in ["PORTAFOGLIO", "WATCHLIST"]:
            evaluate_and_tune(cat)

        # Carica titoli
        portfolio, watchlist, descriptions, etf_list = load_titoli_csv()

        # Analizza solo PORTAFOGLIO e WATCHLIST (no ETF)
        portfolio_results = process_ticker_group(portfolio, "PORTAFOGLIO", descriptions, vix_data)
        watchlist_results = process_ticker_group(watchlist, "WATCHLIST", descriptions, vix_data)
