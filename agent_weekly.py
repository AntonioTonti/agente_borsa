#!/usr/bin/env python3
"""
Agente di Trading - Analisi Settimanale
Invio: Venerdì 18:00 UTC (19:00 IT)
FEATURES:
- ANALISI SETTIMANALE (Heikin Ashi, EMA, RSI, Volume)
- Variazione percentuale settimanale nell'intestazione
- Stima Trend (3 settimane) con Target e Stop Loss
- Pallino riassuntivo e score
- Separazione tra Portafoglio e Watchlist con invii Telegram distinti
"""
...
def analyze_weekly_ticker(ticker: str) -> Tuple[List[str], float, Dict]:
    """
    Analisi settimanale - Calcola indicatori, variazione % settimanale e score.
    ...
    try:
        ...
        # Calcolo Scostamento Settimanale
        if len(close) >= 2:
            last_close = float(close.iloc[-1])
            prev_close = float(close.iloc[-2])
            weekly_var_pct = ((last_close - prev_close) / prev_close) * 100.0
            extra_data['weekly_var_pct'] = weekly_var_pct
...
def create_weekly_report_section(title: str, results: List[Tuple[str, List[str], float, Dict]], descriptions: Dict) -> str:
    """Crea la sezione del report settimanale ordinando per score decrescente."""
    if not results:
        return f"{title} - Nessun segnale oggi"
    
    # Ordinamento dal più alto al più basso score
...
def send_telegram_message(token: str, chat_id: str, message: str, use_markdown: bool = True) -> bool:
    """Invia un messaggio a Telegram con gestione spezzettamento."""
    try:
