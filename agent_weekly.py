#!/usr/bin/env python3
"""
Agente di Trading - Analisi Settimanale (IDENTICA LOGICA DEL GIORNALIERO SU BASE SETTIMANALE)
PESI E SOGLIE AGGIORNATI (TOTALE 100%):
1. EMA10 vs MA31 (18%)
2. Stima Trend 3 sett (15%) [Soglia: ±3.0%]
...
WEEKLY_PERIOD = "2y"      # 2 anni di dati per sufficienti barre
WEEKLY_INTERVAL = "1wk"   # Barre settimanali
WEEKLY_MIN_POINTS = 35    # Minimo 35 settimane per l'analisi completa

def calculate_zigzag_trend(df: pd.DataFrame, deviation_pct: float = 5.0) -> int:
    """
    Calcola l'ultimo trend dell'indicatore ZigZag su timeframe settimanale.
...
def analyze_weekly_ticker(ticker: str) -> Tuple[List[str], float, Dict]:
    """
    Analisi settimanale a 10 parametri identica al giornaliero.
    Le medie trimestrali sono parametrizzate su 12 settimane (~3 mesi).
    """
    signals = []
    ...
    try:
        df = yf.download(ticker, period=WEEKLY_PERIOD, interval=WEEKLY_INTERVAL, auto_adjust=True, progress=False)
        ...
        # 2. STIMA TREND 3 SETTIMANE (PESO 15%)
        if len(close) >= 6:
        ...
        # 3. DELTA % EMA10 vs MA31 PESATO SU MEDIA 3 MESI (~12 SETTIMANE) (PESO 12%)
        if clean_ema is not None and clean_ma is not None and len(clean_ma) >= 12:
            ...
            avg_delta_3m = float(delta_series.tail(12).abs().mean())
        ...
        if len(ha) >= 12:
            ...
            # 4. FORZA CORPO HA VS MEDIA 3 MESI (12 SETTIMANE) (15%)
            ha_bodies = (ha['HA_Close'] - ha['HA_Open']).abs()
            curr_body = ha_bodies.iloc[-1]
            avg_body_3m = float(ha_bodies.tail(12).mean())
        ...
        # 7. VOLUME VS MEDIA 3 MESI (~12 SETTIMANE) (PESO 5%)
        if len(volume) >= 12:
            avg_vol_3m = float(volume.tail(12).mean())
...
def create_portfolio_weekly_report(results: List[Tuple[str, List[str], float, Dict]], descriptions: Dict) -> str:
    if not results:
        return "💰 *PORTAFOGLIO SETTIMANALE* - Nessun segnale oggi"
    
    sorted_results = sorted(results, key=lambda x: x[2], reverse=True)
    lines = ["💰 *PORTAFOGLIO SETTIMANALE*"]
...
def create_watchlist_weekly_report(results: List[Tuple[str, List[str], float, Dict]], descriptions: Dict) -> str:
    if not results:
        return "👁️ *OSSERVATI SETTIMANALI* - Nessun segnale oggi"
    
    sorted_results = sorted(results, key=lambda x: x[2], reverse=True)
    lines = ["👁️ *OSSERVATI SETTIMANALI*"]
...
def main():
    ...
    try:
        print("=" * 60)
        print("📊 AGENTE DI TRADING - ANALISI SETTIMANALE COMPLETA")
        ...
        if token and chat_id:
            if portfolio_results:
                ...
                send_telegram_message(token, chat_id, create_portfolio_weekly_report(portfolio_results, descriptions))
                time.sleep(2)
            if watchlist_results:
                print("\n📩 Invio report Watchlist...")
                send_telegram_message(token, chat_id, create_watchlist_weekly_report(watchlist_results, descriptions))
