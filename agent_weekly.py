...
# Schedule: Tutti i giorni alle 18:00 (6 PM)
# ==========================================
...
def analyze_weekly_ticker(ticker, description):
    ...
    pct_change_wk = ((close.iloc[-1] - close.iloc[-2]) / close.iloc[-2]) * 100

    ema10 = close.ewm(span=10, adjust=False).mean()
    ma31 = close.rolling(window=31).mean()
    if ma31.isna().iloc[-1]:
        ma31 = close.rolling(window=15).mean() # fallback se poche barre
    rsi14 = calculate_rsi(close, 14)
    ha_df = calculate_heikin_ashi(data)

    # LOGICA DI CALCOLO SETTIMANALE ORIGINALE (MANTENUTA COME DA RICHIESTA)
    other_score = 0.50
    ...
    c_ma = ma31.iloc[-1]
    p_ma = ma31.iloc[-2]
    if p_ema < p_ma and c_ema >= c_ma: other_score += 0.25
    elif c_ema > c_ma: other_score += 0.15
    elif p_ema > p_ma and c_ema <= c_ma: other_score -= 0.25
    else: other_score -= 0.15

    # 2. RSI 14 Settimanale
    c_rsi = rsi14.iloc[-1]
    if c_rsi > 70: other_score -= 0.15
    elif c_rsi < 30: other_score += 0.10
    elif c_rsi >= 60: other_score += 0.05
    elif c_rsi <= 40: other_score -= 0.05

    # 3. Volumi vs Media 10 Settimane
    avg_vol_10w = volume.tail(10).mean()
    curr_vol = volume.iloc[-1]
    if curr_vol > 1.5 * avg_vol_10w: other_score += 0.10
    elif curr_vol < 0.5 * avg_vol_10w: other_score -= 0.05

    other_score = max(0.0, min(1.0, other_score))

    # 4. Heikin Ashi Score
    ha_is_green = ha_df['HA_Close'].iloc[-1] >= ha_df['HA_Open'].iloc[-1]
    ha_color_score = 0.35 if ha_is_green else -0.35
    if close.iloc[-1] > close.iloc[-2] and ha_is_green:
        ha_color_score += 0.10
    elif close.iloc[-1] < close.iloc[-2] and not ha_is_green:
        ha_color_score -= 0.10

    ha_normalized = (ha_color_score + 0.45) / 0.9
    ha_normalized = max(0.0, min(1.0, ha_normalized))

    # Score Finale Combinato (35% HA + 65% Altri)
    final_score = (ha_normalized * 0.35) + (other_score * 0.65)
    final_score = max(0.0, min(1.0, final_score))

    return {
        "ticker": ticker,
        "description": description,
        "pct_change_wk": pct_change_wk,
        "score": final_score,
        "ha_normalized": ha_normalized,
...
def run_weekly_agent():
    ...
    for ticker, desc in TICKERS.items():
        res = analyze_weekly_ticker(ticker, desc)
        if res:
            results.append(res)

    # UNIFORMATO: Ordinamento DECRESCENTE per score (più alti in cima)
    results = sorted(results, key=lambda x: x["score"], reverse=True)

    now_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
    report = f"📈 *REPORT SETTIMANALE MACRO-TREND* ({now_str})
"
    report += "----------------------------------------
...
"
    ...
    for r in results:
        sign = "+" if r['pct_change_wk'] >= 0 else ""
        report += f"*{r['ticker']}* - {r['description']} [{sign}{r['pct_change_wk']:.2f}% Wk]
"
        report += f"Score: *({r['score']:.2f})*
"
        report += f"  • Heikin Ashi Score (35%): {r['ha_normalized']:.2f}
"
        report += f"  • Altri Indicatori Score (65%): {r['other_score']:.2f} (RSI: {r['rsi']:.1f})
...
if __name__ == "__main__":
    run_weekly_agent()
weekly_agent.py
Visualizzazione di weekly_agent.py.
